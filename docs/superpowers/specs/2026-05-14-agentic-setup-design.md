# Agentic Workflow Setup — Design Spec
...
Establish a complete agentic workflow harness for the Nightstand repo: project knowledge, guard rails, session state handoff, and permission hygiene. Multiple AI agents (Claude, Codex, opencode) should be able to pick up any session cleanly and operate safely without burning time on discoverable rules or repeating known mistakes.

---

## Decision log

- **AGENTS.md is the single source of truth** for all shared rules. CLAUDE.md contains Claude-specific config only (skills, RTK). This avoids sync drift across agents without requiring a copy script.
- **Option C (reconstruct from scratch)** chosen over copying karavany-crm-fe directly — different stack, different domain, different danger zones.
- **No ai.stack.yaml / AGENTS.md generator** — single flat AGENTS.md is sufficient for this project size.
- opencode format unknown; leave a placeholder, wire up once confirmed.
...
```
nightstand/
├── AGENTS.md                                      ← source of truth (all agents)
├── CLAUDE.md                                      ← Claude-specific only
├── docs/
│   ├── superpowers/specs/                         ← design docs (this file)
│   ├── ai/
│   │   └── project.md                             ← tech stack quick-ref
│   ├── current-state.md                           ← baton-pass state
│   ├── next-task.md                               ← baton-pass next task
│   ├── agent-handoff.md                           ← baton-pass operating doc
│   └── progress.md                                ← append-only session log
├── baton-pass.config.json
├── baton-pass.state.json
└── .claude/
    ├── settings.json                              ← plugins + global denies
    ├── settings.local.json                        ← permission whitelist
    ├── sync-instructions.sh                       ← placeholder (opencode TBD)
    ├── hookify.block-calibredb-embed-metadata.local.md
    ├── hookify.warn-localhost-typo.local.md
    └── commands/
        ├── new-game.md
        ├── foresight.md
        ├── baton-pass.md
...
Nine sections in reading order.
...
- Any Calibre DB write in M1 (enforced by hookify guard)
...
The sidecar is spawned in `lib.rs` via `std::process::Command` pointing to `backend/` — not registered as a Tauri `externalBin`. The spawn command and port-reading logic live entirely in `spawn_sidecar()`.
...
## Section 3: Hookify guards

### `hookify.block-calibredb-embed-metadata.local.md`

Blocks any bash command containing `calibredb embed_metadata`. This command permanently renames files to Calibre's internal format, destroying the normalized filename convention. There is no recovery path.

```yaml
---
name: block-calibredb-embed-metadata
enabled: true
event: bash
pattern: calibredb\s+embed_metadata
action: block
message: |
  calibredb embed_metadata is forbidden in this project.
  It renames files to Calibre's internal format, destroying normalized filenames.
  See AGENTS.md § Forbidden.
---
```

### `hookify.warn-localhost-typo.local.md`

Warns when `127.0.0.1` appears in a frontend fetch call. WebKitGTK blocks this from the webview.

```yaml
---
name: warn-localhost-typo
enabled: true
event: file
conditions:
  - field: file_path
    operator: regex_match
    pattern: frontend/src/.*\.(svelte|ts)$
  - field: new_text
    operator: contains
    value: "127.0.0.1"
action: warn
message: |
  WebKitGTK blocks http://127.0.0.1 from the webview on Linux.
  Use http://localhost instead. See AGENTS.md § Critical gotchas.
---
```
...
Omitted from karavany: prisma, playwright, github, context7 (not applicable here). Add back as needed.

### `.claude/settings.local.json`

Permission whitelist — avoids approval prompts for known-safe operations:
...
## Section 5: Baton-pass system

Copied conceptually from karavany-crm-fe; stack-agnostic. Provides clean session handoff between agents (Claude, Codex, human).

### State files (in `docs/`)

- `current-state.md` — what phase the project is in, what's complete, what's in progress, blockers
- `next-task.md` — the single next task with acceptance criteria, files expected to change, do-not-touch list
- `agent-handoff.md` — operating doc: resume order, source-of-truth hierarchy, repo-specific rules
- `progress.md` — append-only session log; never overwrite, only append

### Config files (in repo root)

- `baton-pass.config.json` — maps trigger names to command names, defines paths to state files, sets rules
- `baton-pass.state.json` — current machine-readable state (state, lastMove, lastAgent, nextAgent, updatedAt, summary)

### Custom commands (in `.claude/commands/`)

| Command | Purpose |
|---------|---------|
| `new-game` | Bootstrap: creates config, state, and all docs/ files |
| `foresight` | Receive session: read state, verify alignment, claim ownership |
| `baton-pass` | Transfer: commit staged work, update docs, write state |
| `save-state` | Pause: checkpoint without transfer |
| `party-check` | Quick status read: state, last agent, next task |
| `dragon-dance` | Record a lesson: problem → impact → new convention |
| `hindsight` | Full audit: baton chain, verification gaps, open risks |
...
- **opencode format** — unknown which instruction file opencode reads. Placeholder in `sync-instructions.sh`. Wire up once confirmed.
- **Testing harness** — backend has no pytest tests yet; frontend has no vitest setup. Both should be added before M2. The "run manually" fallback in workflow rules is temporary.
- **`.gitignore` for docs/** — baton-pass state files may contain in-progress notes; decide whether to track them or gitignore (karavany gitignores them). Recommendation: track them so Codex sessions can see the state.
